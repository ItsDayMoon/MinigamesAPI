...
public class Arena
{
    ...
    public void startLobby(final boolean countdown)
    {
        ...
        this.currenttaskid = Bukkit.getScheduler().runTaskTimer(MinigamesAPI.getAPI(), () -> {
            // ⛔ CANCELAR COUNTDOWN SE FALTAR JOGADOR
            if (Arena.this.getAllPlayers().size() < Arena.this.getMinPlayers()) {
                Arena.this.setArenaState(ArenaState.JOIN);
                Util.updateSign(Arena.this.plugin, Arena.this);
                try {
            ...
            for (String pName : Arena.this.getAllPlayers()) {
     ...
     */
    public boolean alreadyStopped = true;
        ...
        if (!this.temp_delay_stopped)
        {
            if (this.plugin.getConfig().getBoolean(ArenaConfigStrings.CONFIG_DELAY_ENABLED))
            {
                ...
                if (this.plugin.getConfig().getBoolean(ArenaConfigStrings.CONFIG_SPAWN_FIREWORKS_FOR_WINNERS))
                {
                    ...
                    {
                        if(alreadyStopped) {
                            for (String play : this.temp_players) {
                                Arena.this.pli.getRewardsInstance().giveWinReward(play, a, Arena.this.temp_players, Arena.this.global_coin_multiplier);
                            }
                            alreadyStopped = false;
                        }
                        double x = pli.getArenasConfig().getConfig().getDouble("arenas." + a.getInternalName() +".middle.x");
                        double y = pli.getArenasConfig().getConfig().getDouble("arenas." + a.getInternalName() +".middle.y");
                        double z = pli.getArenasConfig().getConfig().getDouble("arenas." + a.getInternalName() +".middle.z");

                        Util.spawnFirework(Bukkit.getPlayer(this.getAllPlayers().get(0)));

                        for (final String player : this.temp_players) {
                            // Teleporta para o centro da arena
                            Player play1 = Bukkit.getPlayer(player);

                            if(play1.getWorld().equals(Bukkit.getWorld("Map_SkyWars"))){
                                Location center = new Location(play1.getWorld(),51.5,106,2.5);
                                play1.teleport(center);
                                this.getFrozenPlayers().add(play1.getUniqueId());
                            }else {
                                Location center = new Location(play1.getWorld(), x, y, z);
                                play1.teleport(center);

                                // Congela o jogador (Slowness e Jump)
                                this.getFrozenPlayers().add(play1.getUniqueId());
                            }// Impede movimentação real (opcional, para segurança extra)/ crie esse Set em algum lugar se quiser usar PlayerMoveEvent
                        }
                    }
                }
                return;
    ...
    public void reset()
    {
        for (Player player : Bukkit.getOnlinePlayers()) {
            if (this.getFrozenPlayers().contains(player.getUniqueId())) {
    ...
    public void onEliminated(final String playername)
    {
        ...
        else
        {
            ...
            // 👇 NOVO: Mensagem de morte sozinho (ex: caiu no void)
            for (final String p_ : this.getAllPlayers())
            {
                if (!p_.equalsIgnoreCase(playername))
                {
                    if (Validator.isPlayerOnline(p_))
                    {
                        Bukkit.getPlayer(p_).sendMessage("§c" + playername + " caiu no void!");
                    }
                }
            }
        }
    ...
    public void endTheMiddle(String p){
