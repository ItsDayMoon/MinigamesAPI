...
public class Classes {
        ...
        if (this.lasticonm.containsKey(p)) {
            ...
            for (final String ac : this.pli.getAClasses().keySet()) {
                ...
                if (ac_.isEnabled()) {
                    ...
                    iconm.setOption(slot, ac_.getIcon().clone(), ac_.getName(),
                            getKitLoreWithStatus(player, ac_.getInternalName(),
                                    pli.getClassesConfig().getConfig().getString("config.kits." + ac_.getInternalName() + ".lore").split(";")));
     ...
     *
     * @param iname  the INTERNAL classname
     * @param player
     */
    public void setClass(String iname, final String player, final boolean money) {
        ...
        if (continue_) {
            ...
            if (classByInternalname != null) {
                ...
                Bukkit.getPlayer(player).sendMessage(set_kit_msg.replaceAll("<kit>", ChatColor.translateAlternateColorCodes('&', classByInternalname.getName())));

                // --- NOVO: Fecha e reabre o menu IMEDIATAMENTE ---
                if (money && this.lasticonm.containsKey(player)) {
                    Player p = Bukkit.getPlayer(player);
                    if (p != null) {
                        this.lasticonm.get(player).destroy(); // Fecha o menu atual
                        this.lasticonm.remove(player); // Remove do cache
                        Bukkit.getScheduler().runTaskLater(plugin, () -> {
                            openGUI(player); // Reabre o menu atualizado
                        }, 2L); // Delay pequeno para evitar conflitos
                    }
                }
            }
        }
    ...
    private String[] getKitLoreWithStatus(Player player, String internalName, String[] originalLore) {
        List<String> lore = new ArrayList<>(Arrays.asList(originalLore));

        // Verifica se o jogador já tem o kit
        boolean hasKit = !kitRequiresMoney(internalName) ||
                (plugin.getConfig().getBoolean(ArenaConfigStrings.CONFIG_BUY_CLASSES_FOREVER) &&
                        pli.getClassesConfig().getConfig().isSet("players.bought_kits." + player.getName() + "." + internalName));

        // Cria a linha de status
        String statusLine;
        if (hasKit) {
            statusLine = ChatColor.GREEN + "&aEquipar Kit";
        } else {
            int price = pli.getClassesConfig().getConfig().getInt("config.kits." + internalName + ".money_amount");
            String currency = plugin.getConfig().getBoolean(ArenaConfigStrings.CONFIG_USE_CREADITS_INSTEAD_MONEY_FOR_KITS)
                    ? "créditos" : "coins";
            statusLine = ChatColor.GOLD + "&6Preço: &f" + price + " " + currency;
        }

        lore.add(""); // Linha vazia para separar
        lore.add(statusLine);
